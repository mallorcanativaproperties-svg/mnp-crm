...
export async function GET(request) {
  ...
  try {
    ...
    // Get active conversations with Idealista leads that might need follow-up
    // States: nuevo, activo, en_curso — NOT derivado, sin_respuesta, cerrado
    ...
    for (const conv of conversaciones || []) {
      ...
      // Only follow up if the last message was from CLAUDIA (client hasn't responded)
      ...
      // Count how many follow-ups have already been sent
      ...
      if (seguimientosPrevios >= MAX_SEGUIMIENTOS) {
        // Already sent 2 follow-ups, mark as sin_respuesta
      ...
      // Save follow-up message in Supabase
      ...
      if (agente) {
        const msgAgente = `⏰ SEGUIMIENTO ${seguimientosPrevios + 1}/2\n\n👤 ${conv.contacto || "Cliente"}\n📱 +${phoneCliente}\n🏠 ${conv.referencia || "N/A"}\n⏱️ ${Math.round(horasSinRespuesta)}h sin responder\n\n${seguimientosPrevios === 0 ? "Le he enviado un primer recordatorio" : "Le he enviado el último recordatorio. Si no contesta, no le insistiré más"}`;
        await sendWhatsApp(agente.telefono, msgAgente);

        // Save agent notification in Supabase (so it shows in CRM)
        ...
        const agentConvId = agentConv?.id;
        if (agentConvId) {
          await supabase.from("mensajes").insert({
