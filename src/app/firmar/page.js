...
Sus datos no serán cedidos a terceros, salvo obligación legal. Los datos son almacenados en servidores seguros (infraestructura AWS, Espacio Económico Europeo).
...
• Permite la identificación del firmante mediante verificación de código, datos personales y documento de identidad.
...
export default function FirmarPage() {
  ...
  async function loadInfo(t) {
    try {
      const res = await fetch("/api/firma", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "info", token: t }) });
  ...
  async function sendCode() {
    ...
    const res = await fetch("/api/firma", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "send_code", token, email }) });
    const data = await res.json();
    setSending(false);
  ...
  async function verifyCode() {
    ...
    const res = await fetch("/api/firma", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "verify_code", token, codigo }) });
    const data = await res.json();
    setSending(false);
  ...
  // Verify DNI number against photo using simple OCR check
  async function verifyDniPhoto(imageData) {
    // We check if the DNI/NIE entered matches what's visible in the photo
    // Using Claude API for OCR
    try {
        ...
        body: JSON.stringify({
          ...
          messages: [{
            ...
            content: [
              ...
              { type: "text", text: `Lee el numero de documento de identidad (DNI, NIE o pasaporte) que aparece en esta imagen. Responde SOLO con el numero/codigo del documento, sin espacios ni guiones, en mayusculas. Si no puedes leerlo responde ILEGIBLE.` }
            ]
          }],
        }),
      });
      const data = await res.json();
      const detected = data.content?.filter(i => i.type === "text").map(i => i.text).join("").trim().toUpperCase().replace(/[\s\-\.]/g, "");
      return detected;
    } catch (e) {
  ...
  async function onDniFrontalCaptured(imageData) {
    setDniFrontal(imageData);
    setDniWarning("");
    // Verify DNI
    setSending(true);
    const detected = await verifyDniPhoto(imageData);
    setSending(false);
    const userDni = dniNie.toUpperCase().replace(/[\s\-\.]/g, "");
    if (detected && detected !== "ILEGIBLE" && !detected.includes(userDni) && !userDni.includes(detected)) {
      setDniWarning(`El documento introducido (${dniNie}) no coincide con el detectado en la foto (${detected}). Verifica que los datos son correctos.`);
    }
  }
  ...
  async function submitSignature() {
    setSending(true); setError("");
    let geo = null;
    try { const pos = await new Promise((res, rej) => navigator.geolocation.getCurrentPosition(res, rej, { timeout: 5000 })); geo = `${pos.coords.latitude},${pos.coords.longitude}`; } catch (e) {}

    let dniFrontalUrl = null, dniDorsoUrl = null;
    try {
      const { createClient } = await import("@supabase/supabase-js");
      const sb = createClient(process.env.NEXT_PUBLIC_SUPABASE_URL, process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY);
      if (dniFrontal) { const blob = await (await fetch(dniFrontal)).blob(); const path = `firmas/${token}/dni_frontal_${Date.now()}.jpg`; await sb.storage.from("propiedades-media").upload(path, blob); const { data } = sb.storage.from("propiedades-media").getPublicUrl(path); dniFrontalUrl = data?.publicUrl; }
      if (dniDorso) { const blob = await (await fetch(dniDorso)).blob(); const path = `firmas/${token}/dni_dorso_${Date.now()}.jpg`; await sb.storage.from("propiedades-media").upload(path, blob); const { data } = sb.storage.from("propiedades-media").getPublicUrl(path); dniDorsoUrl = data?.publicUrl; }
    } catch (e) { console.error("Upload error:", e); }

    const res = await fetch("/api/firma", {
      method: "POST", headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ action: "sign", token, nombre, apellidos, dni_nie: dniNie, firma_img: firmaImg, dni_frontal_url: dniFrontalUrl, dni_dorso_url: dniDorsoUrl, ip: null, user_agent: navigator.userAgent, geolocalizacion: geo }),
    });
    const data = await res.json();
    setSending(false);
  ...
  const inputS = { width: "100%", padding: "14px 16px", border: "2px solid #e0ddd6", borderRadius: 10, fontSize: 14, fontFamily: "'Manrope', sans-serif", outline: "none", marginBottom: 12, boxSizing: "border-box", transition: "border 0.2s", color: "#3d2b1f" };
  ...
  const btnSecondary = { ...btnS, background: "#e8e5de", color: "#5a4a3a" };
  ...
  if (showFullPdf) {
    return (
      <div style={{ position: "fixed", inset: 0, background: "#fff", zIndex: 9999, display: "flex", flexDirection: "column" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 20px", background: "#1a1a1a", color: "#fff" }}>
          <span style={{ fontSize: 13, fontWeight: 600 }}>{info?.pdf_nombre || "Documento"}</span>
          <button onClick={() => setShowFullPdf(false)} style={{ background: "#C8A97E", border: "none", borderRadius: 20, color: "#fff", padding: "8px 24px", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "'Manrope', sans-serif" }}>
            ← Volver a firmar
          </button>
        </div>
        <iframe src={info?.pdf_url} style={{ flex: 1, border: "none", width: "100%" }} />
      </div>
    );
  }
    ...
    <div style={bg}>
      ...
      {(step === "inicio" || step === "email") && (
        <div style={{ ...card, marginBottom: 0 }}>
          <div style={stepInfo}><div style={{ ...stepIcon, background: "#C8A97E22", color: "#C8A97E" }}>①</div><div><strong>Acepta o activa permisos</strong><br/>Durante la firma se podrán solicitar permisos de localización o uso de cámara, estos permisos serán usados únicamente para registrar y realizar el informe de la firma electrónica</div></div>
          <div style={stepInfo}><div style={{ ...stepIcon, background: "#C8A97E22", color: "#C8A97E" }}>②</div><div><strong>Revisa el documento</strong><br/>Lee el documento y revísalo antes de su firma</div></div>
          <div style={stepInfo}><div style={{ ...stepIcon, background: "#C8A97E22", color: "#C8A97E" }}>③</div><div><strong>Acepta todas las condiciones</strong><br/>Para poder firmar deberás aceptar todas las condiciones expuestas</div></div>
          <div style={stepInfo}><div style={{ ...stepIcon, background: "#C8A97E22", color: "#C8A97E" }}>④</div><div><strong>Firmar el documento</strong><br/>Una vez conforme podrás realizar la firma en el documento</div></div>
      ...
      {step === "codigo" && (
        <div style={card}>
          ...
          <p style={ps}>Introduce el código de verificación que te ha proporcionado tu agente.</p>
          ...
          <p style={{ textAlign: "center", fontSize: 12, color: "#888", marginTop: 14, cursor: "pointer" }} onClick={() => { setStep("inicio"); setCodigo(""); setError(""); }}>Volver</p>
        </div>
      )}

      {/* STEP: Document + accept */}
      {step === "documento" && (
      ...
      {/* STEP: Personal data 1/3 */}
      {step === "datos" && (
        <div style={card}>
          <div style={{ fontSize: 12, color: "#888", marginBottom: 4 }}>Paso 1 / 3</div>
          <div style={{ height: 4, background: "#eee", borderRadius: 2, marginBottom: 20 }}><div style={{ width: "33%", height: "100%", background: "#C8A97E", borderRadius: 2 }} /></div>
          ...
          <div style={{ fontSize: 12, fontWeight: 600, color: "#5a4a3a", marginBottom: 4 }}>Nombre</div>
          <input type="text" placeholder="Ej.: Jorge" value={nombre} onChange={(e) => setNombre(e.target.value)} style={inputS} />
          <div style={{ fontSize: 12, fontWeight: 600, color: "#5a4a3a", marginBottom: 4 }}>Apellidos</div>
          <input type="text" placeholder="Ej.: López García" value={apellidos} onChange={(e) => setApellidos(e.target.value)} style={inputS} />
          <div style={{ fontSize: 12, fontWeight: 600, color: "#5a4a3a", marginBottom: 4 }}>DNI / NIE / Pasaporte</div>
      ...
      {step === "firma" && (
        <div style={card}>
          ...
          <div style={{ height: 4, background: "#eee", borderRadius: 2, marginBottom: 20 }}><div style={{ width: "66%", height: "100%", background: "#C8A97E", borderRadius: 2 }} /></div>
          ...
          <div style={{ textAlign: "right", marginBottom: 14 }}><button onClick={clearCanvas} style={{ background: "none", border: "none", color: "#888", fontSize: 12, cursor: "pointer" }}>🗑 Borrar</button></div>
      ...
      {/* STEP: DNI frontal 3/3 */}
      {step === "dni_frontal" && (
        <div style={card}>
          <div style={{ fontSize: 12, color: "#888", marginBottom: 4 }}>Paso 3 / 3 — Documento de identidad</div>
          <div style={{ height: 4, background: "#eee", borderRadius: 2, marginBottom: 20 }}><div style={{ width: "83%", height: "100%", background: "#C8A97E", borderRadius: 2 }} /></div>
          <h2 style={{ ...h2s, color: "#C8A97E" }}>Fotografía frontal del DNI</h2>
          <p style={ps}>Haz una fotografía de la parte frontal de tu documento de identidad. Es obligatorio realizarla en este momento.</p>
          {dniFrontal ? (
            <div>
              <img src={dniFrontal} style={{ width: "100%", borderRadius: 10, marginBottom: 10, border: "1px solid #eee" }} alt="DNI frontal" />
              {sending && <div style={{ textAlign: "center", color: "#C8A97E", fontSize: 12, marginBottom: 10 }}>Verificando documento...</div>}
              {dniWarning && <div style={{ background: "#FEF3C7", border: "1px solid #F59E0B", borderRadius: 8, padding: "10px 14px", marginBottom: 12, fontSize: 12, color: "#92400E" }}>⚠️ {dniWarning}</div>}
              <div style={{ display: "flex", gap: 10 }}>
                <button onClick={() => { setDniFrontal(null); setDniWarning(""); }} style={{ ...btnSecondary, flex: 1 }}>Repetir</button>
                <button onClick={() => setStep("dni_dorso")} disabled={sending} style={sending ? { ...btnDisabled, flex: 2 } : { ...btnS, flex: 2 }}>Siguiente</button>
              </div>
            </div>
          ) : (
            <label style={{ ...btnS, textAlign: "center", display: "block", cursor: "pointer" }}>
              📷 Hacer fotografía
              <input type="file" accept="image/*" capture="environment" style={{ display: "none" }} onChange={(e) => { const file = e.target.files[0]; if (!file) return; const reader = new FileReader(); reader.onload = (ev) => onDniFrontalCaptured(ev.target.result); reader.readAsDataURL(file); }} />
            </label>
          )}
        </div>
      )}
      ...
      {step === "dni_dorso" && (
        <div style={card}>
          <div style={{ fontSize: 12, color: "#888", marginBottom: 4 }}>Paso 3 / 3 — Documento de identidad</div>
          <div style={{ height: 4, background: "#eee", borderRadius: 2, marginBottom: 20 }}><div style={{ width: "95%", height: "100%", background: "#C8A97E", borderRadius: 2 }} /></div>
          <h2 style={{ ...h2s, color: "#C8A97E" }}>Fotografía dorso del DNI</h2>
          <p style={ps}>Haz una fotografía de la parte trasera de tu documento de identidad.</p>
          {dniDorso ? (
            <div>
            ...
            <label style={{ ...btnS, textAlign: "center", display: "block", cursor: "pointer" }}>
              📷 Hacer fotografía
              <input type="file" accept="image/*" capture="environment" style={{ display: "none" }} onChange={(e) => handleCamera(e, setDniDorso)} />
            </label>
          )}
        </div>
      )}
